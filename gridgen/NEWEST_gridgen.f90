...
LOGICAL :: srctrms
REAL(KIND=rDef) :: cRMSr,eRMS,Sres,XeSum,YeSum,xyRMSres,Xsi,Xeta,Ysi,Yeta, &
XeRMS,YeRMS,Xe,Ye
...
DO n = 1,nmax
 ...
 YeSum = 0.0
  ...
  eRMS = SQRT((XeSum+YeSum)/((imax-2)*(jmax-2)*2))

  XeRMS = SQRT((XeSum)/((imax-2)*(jmax-2)))
  YeRMS = SQRT((YeSum)/((imax-2)*(jmax-2)))

  IF (YeRMS <= cRMSr .AND. XeRMS <= cRMSr) THEN
    WRITE(6,*) n,XeRMS,YeRMS
    WRITE(6,'(A)') ' '
    WRITE(6,'(A,I3,A)') 'Met convergence criteria in ',n,' iterations'
    WRITE(6,'(A)') 'RMS residual error history above: iteration,eRMS,XeRMS,YeRMS'
