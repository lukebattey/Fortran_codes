PROGRAM elliptic_grid_gen
    ...
    REAL(KIND=rDef),ALLOCATABLE,DIMENSION(:,:) :: XNext,YNext,xResidNext,yResidNext

    REAL(KIND=rDef),ALLOCATABLE,DIMENSION(:,:) :: psi,phi
    CHARACTER(len=30) :: infile,outfile
    CHARACTER(len=8) :: junk
    LOGICAL :: srctrms
    REAL(KIND=rDef) :: cRMSr,RMSres,Sres
    ...
    ALLOCATE(X(imax,jmax),Xsi(imax,jmax),Xsisi(imax,jmax),Xeta(imax,jmax), &
        Xetaeta(imax,jmax), &
        Y(imax,jmax),Ysi(imax,jmax),Ysisi(imax,jmax),Yeta(imax,jmax), &
        Yetaeta(imax,jmax), &
        alpha(imax,jmax),beta(imax,jmax),gama(imax,jmax), &
        bbx(imax,jmax),ddx(imax,jmax),aax(imax,jmax),ccx(imax,jmax), &
        bby(imax,jmax),ddy(imax,jmax),aay(imax,jmax),ccy(imax,jmax), &
        xResid(imax,jmax),yResid(imax,jmax))
    ALLOCATE(XNext(imax,jmax),YNext(imax,jmax),xResidNext(imax,jmax),yResidNext(imax,jmax))
    ...
    DO n = 1,nmax
        ...
        DO j=2,jmax-1
            ...
            DO i = 2,imax-1
                ...
                Sres = Sres + (xResidNext(i,j) - xResid(i,j))**2 + (yResidNext(i,j) - yResid(i,j))**2
            END DO

        END DO !---->  j loop

        RMSres = SQRT(Sres/((imax-2)*(jmax-2)*2)) 

        IF (RMSres <= cRMSr) THEN
            WRITE(6,*) n,RMSres
            WRITE(6,'(A)') ' '
    ...
    !-----------------------------Just in case -----------------------------------------------------
    IF (n >= nmax .AND. RMSres > cRMSr) THEN
