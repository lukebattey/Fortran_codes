...
DO j = 2,jmax-1    !<--- LOOPS ARE ALSO DIFFERENT FOR G FLUXES...
    DO i = 1,imax-1
        ...
        pL(i,j) = (gama-1.0)*(ULF(i,j,4) - (ULF(i,j,2)**2 + ULF(i,j,3)**2) / &
                  (2.0*ULF(i,j,1)))

        pR(i,j) = (gama-1.0)*(URF(i,j,4) - (URF(i,j,2)**2 + URF(i,j,3)**2) / &
                  (2.0*URF(i,j,1)))

        h0L(i,j) = pL(i,j)*gama/(ULF(i,j,1)*(gama-1.0)) + &
        0.5*((ULF(i,j,2)/ULF(i,j,1))**2 + (ULF(i,j,3)/ULF(i,j,1))**2)

        h0R(i,j) = pR(i,j)*gama/(URF(i,j,1)*(gama-1.0)) + &
        0.5*((URF(i,j,2)/URF(i,j,1))**2 + (URF(i,j,3)/URF(i,j,1))**2)
