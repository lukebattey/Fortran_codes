...
SUBROUTINE write_sol
    ...
    WRITE(36,'(A)') 'VARIABLES = "X" "Y" "p" "rho" "T" "u" "v" "M"'
    WRITE(36,'(A,I3,A,I3)') 'ZONE I= ',imax,'    J=  ',jmax
    DO j=1,jmax
        DO i=1,imax
            WRITE(36,*) X(i,j),Y(i,j),p(i,j),rho(i,j),T(i,j),u(i,j),v(i,j),M(i,j)
        END DO
    END DO
    CLOSE(36)
    ...
    WRITE(6,*) ' '
    WRITE(6,'(A,I7,A,F9.4,A)') 'Computed',n,' iterations in',RunTime," seconds" 
    WRITE(6,'(A)') 'Iteration vs. Residual RMSe history printed above'
    WRITE(6,*) 'primatives wrote to:  ',outfile
...
SUBROUTINE pressure_check 
    ...
    DO i=1,imax
        IF (ABS(p(i,1)-pcheck(i)) .gt. 1e-10) THEN
            WRITE(6,*) "boundary p changed from",pcheck(i),p(i,1)
            STOP 124134198
        END IF
    END DO
